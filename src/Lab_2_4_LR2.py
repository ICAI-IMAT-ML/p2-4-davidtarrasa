...
class LinearRegressor:
    ...
    def fit_gradient_descent(self, X, y, learning_rate=0.01, iterations=1000):
        ...
        # Convertimos los coeficientes a un array NumPy
        self.coefficients = np.array(self.coefficients, dtype=np.float64)
        ...
        for epoch in range(iterations):            
            ...
            d_coefficients = (2 / m) * np.dot(X[:, 1:].T, error)
...
def one_hot_encode(X, categorical_indices, drop_first=False):
    ...
    for index in sorted(categorical_indices, reverse=True):
        ...
        one_hot = np.array([[1 if element == value else 0 for value in unique_values] for element in categorical_column])
